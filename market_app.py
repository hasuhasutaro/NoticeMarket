...
class MarketApp(tk.Tk):
    ...
    def __init__(self):
        ...
        self.cmb_item.configure(background="#23272a", foreground="#f5f6fa")
        # 最大価格
        ...
        self.lst_conditions.pack(fill=tk.X, padx=4, pady=4)
    ...
    def on_item_selected(self, event):
        # 選択時にitemIdを内部で保持するだけ
        sel = self.cmb_item.get()
        self.selected_itemid = self.itemid_map.get(sel, "")

    def add_condition(self):
        # 入力値が候補に完全一致していればitemidを自動取得
        sel = self.cmb_item.get().strip()
